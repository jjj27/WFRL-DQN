...
the_model = torch.load('./tmp/100-0.4Epig.pth', map_location='cpu')

dqn = DQN()
dqn.load_state_dict(the_model)

# env = Environment(taskCount=100, alpha=0.8)
# DAG = XMLtoDAG("./ScientificWorkflow/Montage_100.xml").getDAG()
DAG = XMLtoDAG("./ScientificWorkflow/Epigenomics_100.xml").getDAG()
SCWorklfow = Workflow(taskCount=100, alpha=0.4, DAG=DAG)
env = Environment(taskCount=100, save=False, alpha=0.4, workflow=SCWorklfow)
done = False
...
for i in range(100):
    ...
    montageWorklfow = Workflow(taskCount=100, alpha=0.4, DAG=DAG)
    env = Environment(taskCount=100, save=False, alpha=0.4, workflow=montageWorklfow)
    # env.workflow.print()
